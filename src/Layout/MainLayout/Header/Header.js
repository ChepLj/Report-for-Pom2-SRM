...
export default function Header({ user, authLogin }) {
   ...
   authLogin.photoURL ??= 'https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcS_h6QOa3uNotOm0ue_LW0AABchkoq3MtRadKl6pFU&s';
      ...
      if (inputText.match(passw)) {
         ...
         if (inputText === inputText2) {
            ...
            getFirebaseData('UserID').then((data) => {
               ...
               for (const user in UserIDList) {
                  if (user === inputText) {
                     //:kiểm tra nếu đã tồn tại thì không cho phép thêm và thoát vòng lặp
                     return alert(
                        'Mã định danh này không được chấp nhận, vui lòng thay bằng mã định danh khác !\nhoặc liên hệ Mr.Sỹ để biết thêm chi tiết ',
                     );
                  }
               }
               ...
               createPostData(`UserID/${inputText}`, objectPost).then(() => {
                  uploadCallback();
               });
               ////////////////////////////////////
            });
                  ...
                  {authLogin?.displayName === 'guest' ? (
                     ...
                     <>
                        ...
                        <div className={style.userIDWrap}>
                           <span className={style.userID} onClick={handelViewUserID}>
                              Xem mã định danh
                           </span>
                           <span className={style.userID} onClick={handelChangeUserID}>
                              Đổi mã định danh
                           </span>
                        </div>
                        <button
